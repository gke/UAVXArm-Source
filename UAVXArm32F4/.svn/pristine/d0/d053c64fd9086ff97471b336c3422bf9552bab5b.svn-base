...
void DoTesting(void) {
	...
	while (1) {
		...
		if (tick % ratio == 0) {
			...
			BaroNoise = SensorNoise(1.0f);

			RawDensityAltitude = real32Median3Filter(&BaroM3F, FakeAltitude
					+ BaroNoise);
			...
			DensityAltitude2 = LPFn(&AltitudeLPF, FakeAltitude + BaroNoise,
					AltdT);
			...
			TxVal32(TelemetrySerial, T * 1000.0f, 3, ',');
			TxVal32(TelemetrySerial, TrackBaroVariance * 1000.0f, 3, ',');
			TxVal32(TelemetrySerial, TrackAccUVariance * 1000.0f, 3, ',');
			TxVal32(TelemetrySerial, EstAccUBias * 1000.0f, 3, ',');

			TxVal32(TelemetrySerial, FakeAltitude * 1000.0f, 3, ',');
			TxVal32(TelemetrySerial, RawDensityAltitude * 1000.0f, 3, ',');
			TxVal32(TelemetrySerial, DensityAltitude * 1000.0f, 3, ',');
			TxVal32(TelemetrySerial, DensityAltitude2 * 1000.0f, 3, ',');
