...
void CalibrateAccZeros(uint8 s, uint8 imuSel) {
	...
	Config.AccCal.TRef = t * SamplesR;

	for (c = X; c <= Z; c++) {
		Config.AccCal.Scale[c] = DEF_ACC_SCALE;
		Config.AccCal.Bias[c] = a[c] * SamplesR;
	}
...
void CalibrateAccAndGyro(uint8 s, uint8 imuSel) {
	...
	for (c = X; c <= Z; c++) {
		...
		Config.AccCal.Scale[c] = DEF_ACC_SCALE;
		Config.AccCal.Bias[c] = Config.GyroCal.M[c] = Config.GyroCal.C[c]
				= 0.0f;
	}
...
#else
		GyroBias[a] = Config.GyroCal.C[a] + Config.GyroCal.M[a]
				* (MPU6XXXTemperature - Config.GyroCal.TRef);
#endif
} // UpdateGyroTempComp
